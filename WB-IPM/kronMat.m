classdef kronMat
  ...
  methods
    ...
    function A = full(arg1) % Constructs full matrix
      if iscell(arg1.B)
        A = 0;
        for i = 1:length(arg1.B)
          A = A + kron(arg1.B{i},arg1.C{i});
        end
      else
        A = kron(arg1.B,arg1.C);
      end
    end
    ...
    function y = mldivide(arg1, arg2) % Overload matrix inverse
      %   Implement A\s for kronMat object A and "vector" s
      %   This only works if A = B kron C and does not work if we have a
      ...
      if isa(arg1,'kronMat')
        ...
        if iscell(arg1.B)
          error('Inversion with a sum is not implemented.')
        else
          [m1, m2] = size(arg1.C);
          [n1, n2] = size(arg1.B);
          
          if arg1.transpose
            % Matrix transpose inversion
            y = arg1.C' \ reshape(arg2,m1,n1) / arg1.B;
          else
            % Matrix inversion (B x C)^{-1} = B^{-1} x C^{-1}
            y = arg1.C \ reshape(arg2,m2,n2) / arg1.B';
          end
          y = y(:);
        end
      else
        error('Inversion is not implemented on right.')
      end
    end
