function G=amanatidesWooAlgorithm(source, receiver, grid3D, verbose)
...
origin = source;
vec = receiver - source;
direction = [vec'*a/norm(vec)/norm(a);...
    vec'*b/norm(vec)/norm(b);...
    vec'*c/norm(vec)/norm(c);];
...
%         figure;
        ...
        plot3(receiver(1),receiver(2),receiver(3),'k.','MarkerSize',20);
        quiver3(origin(1), origin(2), origin(3), direction(1), direction(2), direction(3), norm(vec));
    ...
    if (flag==0)
        ...
        while ( (x<=grid3D.nx)&&(x>=1) && (y<=grid3D.ny)&&(y>=1) && (z<=grid3D.nz)&&(z>=1) )
           ...
           rx=x;ry=y;rz=z;
            ...
            if dlength~=0
            grid3D.G(ry,rx,rz)=dlength;
            end
            
             if (verbose&&dlength~=0)
                %fprintf('\nIntersection: voxel = [%d %d %d]', [rx ry rz]);
                %fprintf('\nIntersection: length = %d ', dlength);
                   

                t1 = [(rx-1)/grid3D.nx, (ry-1)/grid3D.ny, (rz-1)/grid3D.nz ]';
                t2 = [  (rx)/grid3D.nx,  (ry)/grid3D.ny,    (rz)/grid3D.nz ]';        
